...
final class NFCoreWorkflowDialogModelTests: XCTestCase {
    ...
    func testFetchngsExplainsAccessionInputsAndUsesPinnedVersion() throws {
        ...
        XCTAssertEqual(model.selectedWorkflowDetail.title, "Download public sequencing reads")
        XCTAssertEqual(model.selectedWorkflowDetail.overview, "Download FASTQ files and sample information from public accession lists.")
        XCTAssertTrue(model.selectedWorkflowDetail.requiredInputs.contains("accession list"))
        XCTAssertTrue(model.selectedWorkflowDetail.whenToUse.contains("SRA, ENA, DDBJ, GEO, or Synapse"))
    ...
    func testFirstWaveWorkflowDetailsUsePlainLanguageTitlesAndExamples() {
        let model = NFCoreWorkflowDialogModel(projectURL: nil)

        for workflow in model.availableWorkflows {
            model.selectWorkflow(named: workflow.name)

            XCTAssertFalse(model.selectedWorkflowDetail.title.localizedCaseInsensitiveContains("nf-core"), workflow.name)
            XCTAssertFalse(model.selectedWorkflowDetail.title.localizedCaseInsensitiveContains(workflow.name), workflow.name)
            XCTAssertFalse(model.selectedWorkflowDetail.overview.localizedCaseInsensitiveContains("Nextflow"), workflow.name)
