...
cmds=$(sed -n '/^SUBCOMMANDS:/,/^$/p' "$OUT/_root.txt" | grep -E '^  [a-z0-9]' | awk '{print $1}')
for c in $cmds; do
  f="$OUT/$c.txt"
  ...
  subs=$(sed -n '/^SUBCOMMANDS:/,/^$/p' "$f" | grep -E '^  [a-z0-9]' | awk '{print $1}' | sed 's/^(default)$//' || true)
  for s in $subs; do
    { echo; echo "==== $c $s ===="; timeout 30 "$CLI" help "$c" "$s" 2>&1; } >> "$f"
    # one more level for the groups that nest (fastq, tree infer, genotype)
    subsubs=$(sed -n "/==== $c $s ====/,\$p" "$f" | sed -n '/^SUBCOMMANDS:/,/^$/p' | grep -E '^  [a-z0-9]' | awk '{print $1}' || true)
