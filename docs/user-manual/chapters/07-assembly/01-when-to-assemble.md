...
Lungfish Genome Explorer (LGE) ships five assemblers behind one shared configuration sheet. You reach the sheet by picking a tool under **Tools > Assembly**, and it opens with that tool already chosen in its Assembler picker. Whichever one runs, the result is stored in a per-run folder under the project's `Analyses/` folder. LGE shows that result as one sidebar item, so you open the result row rather than looking for a nested bundle. `Analyses/` is likewise a folder, visible in the LGE sidebar rather than something you type.

The assembly result holds the contig sequences and the run's records. Use **Create Bundle** to turn selected contigs into a `.lungfishref` [reference bundle](../../GLOSSARY.md#reference-bundle), the format LGE uses for a genome you work against. Those later steps are the ones the rest of this manual covers, chiefly viewing a contig as a sequence, mapping fresh reads back onto it, and calling variants against it.
...
There is no threshold that separates a good N50 from a bad one across all genomes, and any number offered as one is wrong. What you compare against is the genome you were trying to assemble. An N50 near the full genome length on a small circular genome, as above, means it came back whole. An N50 of 50 kb on a bacterial chromosome of 5 Mb is a perfectly ordinary short-read result. The same 50 kb on an expected genome of 30 kb needs review. An assembler can duplicate sequence, especially around a circular genome, or the reads may contain a different organism from the one you expected.

## Where the result lands

Every assembler writes an assembly result into a per-run folder under the project's `Analyses/` folder. The folder is named for the tool and the moment the run started, and LGE writes that name for you. It reads as the year, then the month, then the day, then a `T`, then the hour, minute, and second, so a run started at ten past two in the afternoon on 7 September 2026 gives `spades-2026-09-07T14-23-10`. Nothing there is typed by you. A run across several bundles at once uses `spades-batch-` followed by the same timestamp, with the individual assemblies inside it.
...
The result appears in the sidebar as one item rather than as a list of contigs. Opening it opens the assembly viewport. Read the whole-assembly figures in the Inspector's **Bundle** tab under **Assembly Context**, which lists the assembler, read type, contig count, total bp, N50, L50, longest contig, and global GC percent. Here bp is short for base pairs and means the same as bases everywhere else in this chapter. The block also shows the tool version and wall time when recorded. Wall time is the elapsed real time the run took from start to finish. The current Preview may show only wall time in the strip above the table, so use the Inspector for the complete set.

The viewport table lists the contigs one per row, ranked longest first, with columns for rank, contig name, length in bases, GC percent, share of the assembly as a percentage, and a preview of the sequence. The share is of the total assembled bases rather than of the contig count, so on a single-contig assembly it reads 100%. The preview shows the contig's first 80 bases, which is enough to recognise a sequence and not enough to read one, so anything more comes from the detail pane. Selecting a row fills that detail pane beside the table with the contig's header, its length, its GC, its rank, its share, and its full sequence.
...
4. The tool that ran is the tool you meant. The Inspector's Assembly Context block names the assembler and its version, which matters most after a run where the picker narrowed its choices because the detected read class ruled some tools out.
