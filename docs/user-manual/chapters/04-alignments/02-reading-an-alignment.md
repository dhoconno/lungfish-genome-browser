...
shots:
  ...
  - id: pileup-zoom
    caption: "The pileup at position 2,078 of the HG002 chromosome 20 slice, with matching bases drawn as dots and the alternate A drawn as a coloured letter on both strands."
...
**Show matching bases as dots.** Draws a base that agrees with the reference as a dot and one that disagrees as a coloured letter, so the disagreements are the only letters on screen. It is on by default, because a screen of letters where all but three are the same letter hides the three. Turn it off when you want to read the actual sequence of a read rather than only where it differs, and every base is drawn as a letter with the matches in grey.
