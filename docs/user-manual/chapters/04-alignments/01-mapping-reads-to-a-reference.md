...
Five numbers sit at the top. Total Mapped is how many alignment records were placed on the reference. A record is one row of the BAM, and it is not quite the same thing as a read, because a read that the mapper splits across two places contributes two rows. The Flag Statistics list below settles the difference for this fixture. Total Unmapped is how many records were not placed. Mapped % is the first as a percentage of the two together, drawn as a bar beneath the figure. Chromosomes counts the reference sequences the BAM's header names, which is 1 for this fixture because the reference holds only the chromosome 20 slice. Est. Coverage is the estimated average depth across that sequence, worked out as the total mapped bases divided by the length of the reference, and it appears only when the alignment has a single contig, so a whole-genome reference shows no such row.

For the fixture mapped with minimap2 those five read 90,990, 213, 99.8%, 1, and 44.7x. Depth is the number of reads covering a single position, which is the figure Est. Coverage averages, and it is a different measurement from breadth, which counts how many positions carry any read at all. On the HG002 slice the mean depth is 44.7x, inside the 30x to 50x band a human genome project usually aims for. A region under 10x is too thin to call a variant with confidence.
...
Confirm the depth is enough for what comes next. Est. Coverage of 44.7x on this fixture is comfortable for variant calling. Anything under about 10x leaves too little evidence to separate a real difference from an instrument error.
