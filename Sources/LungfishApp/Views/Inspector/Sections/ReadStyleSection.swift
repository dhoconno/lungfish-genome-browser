...
public final class ReadStyleSectionViewModel {
    ...
    // MARK: - Mapped-Read Annotation Inspector State

    /// Currently selected source alignment for mapped-read annotation conversion.
    public var selectedMappedReadsAnnotationSourceTrackID: String? = nil {
        didSet {
            refreshMappedReadsAnnotationOutputTrackNameIfNeeded()
        }
    }

    /// Output annotation track name for mapped-read conversion.
    public var mappedReadsAnnotationOutputTrackName: String = ""

    /// Output annotation track ID for mapped-read conversion.
    public var mappedReadsAnnotationOutputTrackID: String = ""

    /// Whether to skip secondary and supplementary alignments during conversion.
    public var mappedReadsAnnotationPrimaryOnly: Bool = false

    /// Whether to include SAM SEQ in generated annotation attributes.
    public var mappedReadsAnnotationIncludeSequence: Bool = false

    /// Whether to include SAM QUAL in generated annotation attributes.
    public var mappedReadsAnnotationIncludeQualities: Bool = false

    /// Whether to replace an existing annotation track with the same generated ID or name.
    public var mappedReadsAnnotationReplaceExisting: Bool = false

    /// Whether the mapped-read annotation workflow is currently running.
    public var isMappedReadsAnnotationWorkflowRunning: Bool = false

    /// Most recent mapped-read annotation outcome message.
    public var latestMappedReadsAnnotationMessage: String? = nil

    /// Called to convert mapped reads from the selected alignment into annotations.
    public var onConvertMappedReadsToAnnotationsRequested: ((MappedReadsAnnotationInspectorLaunchRequest) -> Void)?

    private var lastAutoGeneratedMappedReadsAnnotationOutputTrackName: String = ""
    private var lastAutoGeneratedMappedReadsAnnotationOutputTrackID: String = ""
    ...
    public func configureAlignmentFilterTracks(_ options: [AlignmentFilterTrackOption]) {
        ...
        let hadValidMappedReadsSelection = selectedMappedReadsAnnotationSourceTrackID.flatMap { current in
            options.first(where: { $0.id == current })
        } != nil
        ...
        if let current = selectedMappedReadsAnnotationSourceTrackID,
           options.contains(where: { $0.id == current }) == false {
            selectedMappedReadsAnnotationSourceTrackID = nil
        }
        ...
        if selectedMappedReadsAnnotationSourceTrackID == nil {
            selectedMappedReadsAnnotationSourceTrackID = options.first?.id
        } else if hadValidMappedReadsSelection {
            refreshMappedReadsAnnotationOutputTrackNameIfNeeded()
        }
    }
    ...
    /// Builds a validated Inspector-local launch request for mapped-read annotation conversion.
    public func makeMappedReadsAnnotationLaunchRequest() throws -> MappedReadsAnnotationInspectorLaunchRequest {
        guard let sourceTrackID = selectedMappedReadsAnnotationSourceTrackID,
              alignmentFilterTrackOptions.contains(where: { $0.id == sourceTrackID }) else {
            throw MappedReadsAnnotationInspectorValidationError.missingSourceTrackSelection
        }

        let trimmedOutputName = mappedReadsAnnotationOutputTrackName.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmedOutputName.isEmpty else {
            throw MappedReadsAnnotationInspectorValidationError.missingOutputTrackName
        }

        let trimmedOutputTrackID = mappedReadsAnnotationOutputTrackID.trimmingCharacters(in: .whitespacesAndNewlines)
        let outputTrackID = trimmedOutputTrackID.isEmpty ? nil : trimmedOutputTrackID
        if let outputTrackID, !Self.isPortableAnnotationTrackID(outputTrackID) {
            throw MappedReadsAnnotationInspectorValidationError.invalidOutputTrackID(outputTrackID)
        }

        return MappedReadsAnnotationInspectorLaunchRequest(
            sourceTrackID: sourceTrackID,
            outputTrackName: trimmedOutputName,
            outputTrackID: outputTrackID,
            primaryOnly: mappedReadsAnnotationPrimaryOnly,
            includeSequence: mappedReadsAnnotationIncludeSequence,
            includeQualities: mappedReadsAnnotationIncludeQualities,
            replaceExisting: mappedReadsAnnotationReplaceExisting
        )
    }
    ...
    private func resetMappedReadsAnnotationState() {
        selectedMappedReadsAnnotationSourceTrackID = nil
        mappedReadsAnnotationOutputTrackName = ""
        mappedReadsAnnotationOutputTrackID = ""
        mappedReadsAnnotationPrimaryOnly = false
        mappedReadsAnnotationIncludeSequence = false
        mappedReadsAnnotationIncludeQualities = false
        mappedReadsAnnotationReplaceExisting = false
        isMappedReadsAnnotationWorkflowRunning = false
        lastAutoGeneratedMappedReadsAnnotationOutputTrackName = ""
        lastAutoGeneratedMappedReadsAnnotationOutputTrackID = ""
    }
    ...
    /// Stores a plain-language outcome message for mapped-read annotation conversion.
    public func noteMappedReadsAnnotationCreation(createdTrackName: String, sourceTrackName: String) {
        latestMappedReadsAnnotationMessage =
            "Created annotation track \(createdTrackName) from mapped reads in \(sourceTrackName)."
    }
    ...
    private func refreshMappedReadsAnnotationOutputTrackNameIfNeeded(force: Bool = false) {
        let suggestedName = defaultMappedReadsAnnotationOutputTrackName()
        guard !suggestedName.isEmpty else { return }
        let suggestedTrackID = Self.defaultAnnotationTrackID(from: suggestedName)

        let currentName = mappedReadsAnnotationOutputTrackName.trimmingCharacters(in: .whitespacesAndNewlines)
        if force || currentName.isEmpty || currentName == lastAutoGeneratedMappedReadsAnnotationOutputTrackName {
            mappedReadsAnnotationOutputTrackName = suggestedName
        }
        lastAutoGeneratedMappedReadsAnnotationOutputTrackName = suggestedName

        let currentTrackID = mappedReadsAnnotationOutputTrackID.trimmingCharacters(in: .whitespacesAndNewlines)
        if force || currentTrackID.isEmpty || currentTrackID == lastAutoGeneratedMappedReadsAnnotationOutputTrackID {
            mappedReadsAnnotationOutputTrackID = suggestedTrackID
        }
        lastAutoGeneratedMappedReadsAnnotationOutputTrackID = suggestedTrackID
    }

    private func defaultMappedReadsAnnotationOutputTrackName() -> String {
        guard let sourceTrackID = selectedMappedReadsAnnotationSourceTrackID,
              let option = alignmentFilterTrackOptions.first(where: { $0.id == sourceTrackID }) else {
            return ""
        }
        return "\(option.name) mapped reads"
    }

    private static func defaultAnnotationTrackID(from name: String) -> String {
        let allowed = CharacterSet.alphanumerics.union(CharacterSet(charactersIn: "_-"))
        let lowercased = name.lowercased()
        let normalizedScalars = lowercased.unicodeScalars.map { scalar -> UnicodeScalar in
    ...
    private static func isPortableAnnotationTrackID(_ value: String) -> Bool {
        let allowed = CharacterSet.alphanumerics.union(CharacterSet(charactersIn: "_-"))
...
public struct MappedReadsAnnotationInspectorLaunchRequest: Equatable {
    public let sourceTrackID: String
    public let outputTrackName: String
    ...
    public func workflowRequest(bundleURL: URL) -> LungfishWorkflow.MappedReadsAnnotationRequest {
        LungfishWorkflow.MappedReadsAnnotationRequest(
            bundleURL: bundleURL,
            sourceTrackID: sourceTrackID,
...
public enum MappedReadsAnnotationInspectorValidationError: Error, LocalizedError, Equatable {
    case missingSourceTrackSelection
    case missingOutputTrackName
    ...
    public var errorDescription: String? {
        switch self {
        case .missingSourceTrackSelection:
            return "Choose a source alignment before converting mapped reads."
        case .missingOutputTrackName:
            return "Enter a name for the annotation track."
        case .invalidOutputTrackID(let id):
            return "Annotation track ID '\(id)' can only contain letters, numbers, underscores, and hyphens."
        }
    }
}
...
public struct AnalysisSection: View {
    ...
    @State private var mappedReadsAnnotationValidationMessage: String?
    ...
    private var mappedReadsAnnotationSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Picker(
                "Source Alignment",
                selection: Binding(
                    get: { viewModel.selectedMappedReadsAnnotationSourceTrackID ?? "" },
                    set: { newValue in
                        mappedReadsAnnotationValidationMessage = nil
                        viewModel.selectedMappedReadsAnnotationSourceTrackID = newValue.isEmpty ? nil : newValue
                    }
                )
            ) {
                if viewModel.alignmentFilterTrackOptions.isEmpty {
            ...
            Toggle("Use primary alignments only", isOn: Binding(
                get: { viewModel.mappedReadsAnnotationPrimaryOnly },
                set: { newValue in
                    mappedReadsAnnotationValidationMessage = nil
                    viewModel.mappedReadsAnnotationPrimaryOnly = newValue
                }
            ))

            Toggle("Include read sequence", isOn: Binding(
                get: { viewModel.mappedReadsAnnotationIncludeSequence },
                set: { newValue in
                    mappedReadsAnnotationValidationMessage = nil
                    viewModel.mappedReadsAnnotationIncludeSequence = newValue
                }
            ))

            Toggle("Include read qualities", isOn: Binding(
                get: { viewModel.mappedReadsAnnotationIncludeQualities },
                set: { newValue in
                    mappedReadsAnnotationValidationMessage = nil
                    viewModel.mappedReadsAnnotationIncludeQualities = newValue
                }
            ))

            Toggle("Replace existing track", isOn: Binding(
                get: { viewModel.mappedReadsAnnotationReplaceExisting },
                set: { newValue in
                    mappedReadsAnnotationValidationMessage = nil
                    viewModel.mappedReadsAnnotationReplaceExisting = newValue
                }
            ))
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                TextField(
                    ...
                    text: Binding(
                        get: { viewModel.mappedReadsAnnotationOutputTrackName },
                        set: { newValue in
                            mappedReadsAnnotationValidationMessage = nil
                            viewModel.mappedReadsAnnotationOutputTrackName = newValue
                        }
                    )
                )
                .textFieldStyle(.roundedBorder)
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                TextField(
                    "annotation_track_id",
                    text: Binding(
                        get: { viewModel.mappedReadsAnnotationOutputTrackID },
                        set: { newValue in
                            mappedReadsAnnotationValidationMessage = nil
                            viewModel.mappedReadsAnnotationOutputTrackID = newValue
                        }
                    )
                )
                .textFieldStyle(.roundedBorder)
            ...
            Button("Convert Mapped Reads to Annotations") {
                do {
                    let request = try viewModel.makeMappedReadsAnnotationLaunchRequest()
                    mappedReadsAnnotationValidationMessage = nil
                    viewModel.onConvertMappedReadsToAnnotationsRequested?(request)
                } catch {
                    mappedReadsAnnotationValidationMessage = error.localizedDescription
                }
            }
            .disabled(
                viewModel.isMappedReadsAnnotationWorkflowRunning ||
                !viewModel.hasAlignmentTracks
            )

            if viewModel.isMappedReadsAnnotationWorkflowRunning {
                HStack(spacing: 6) {
                    ProgressView()
                        .scaleEffect(0.7)
