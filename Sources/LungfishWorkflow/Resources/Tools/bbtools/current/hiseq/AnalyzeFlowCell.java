...
//			if(verbose){outstream.println("Finished; closing streams.");}
	...
	void fillTiles(){
		...
		//Close the read streams
		errorState|=ReadWrite.closeStreams(cris, sidechannel==null ? null : sidechannel.cros);
	}
...
//				if(verbose){outstream.println("Returned a list.");} //Disabled due to non-static access
		...
		void processList(ListNum<Read> ln){
			...
			ArrayList<Read> alignedList=(sidechannel==null ? null : new ArrayList<Read>());
			//Loop through each read in the list
			for(int idx=0; idx<reads.size(); idx++){
				final Read r1=reads.get(idx);
				final Read r2=r1.mate;
				...
				if(alignedList!=null && (r1.mapped() || r1.mateMapped())) {
					alignedList.add(r1);
				}
			}
			if(sidechannel!=null) {sidechannel.write(alignedList, ln.id);}
		}
