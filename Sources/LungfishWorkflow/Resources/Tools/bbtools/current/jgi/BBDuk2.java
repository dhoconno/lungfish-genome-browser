...
//						ns++;
					...
					if(verify){
						...
						assert(shared.Vector.sum(countCounts)>0 && (shared.Vector.sum(countCounts)<=window+1)): current+", "+Tools.cardinality(counts)+"\n"+Arrays.toString(countCounts);
					}
...
//						ns--;
					...
					if(verify){
						assert(current==Tools.cardinality(counts)) : current+", "+Tools.cardinality(counts)+"\n"+Arrays.toString(counts);
						assert(shared.Vector.sum(countCounts)>=0 && (shared.Vector.sum(countCounts)<=window)): current+", "+Tools.cardinality(counts)+"\n"+Arrays.toString(countCounts);
					}
...
//					System.err.println("Removed "+kmer2+"; count="+counts[kmer2]);
				...
				if(verify && i2>-1 && i<bases.length){
					...
					assert(shared.Vector.sum(countCounts)==window): current+", "+Tools.cardinality(counts)+"\n"+Arrays.toString(countCounts);
				}
