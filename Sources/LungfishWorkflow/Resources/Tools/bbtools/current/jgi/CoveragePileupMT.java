...
//		assert(false) : qtrimLeft+", "+qtrimRight+", "+trimq+", "+trimE;
		...
		stdin=FileFormat.isStdio(inputFiles.get(0));
		if(stdin) {
			assert(inputFiles.size()==1) : "stdin must be the only input: "+inputFiles.get(1);
//			throw new RuntimeException(
//					"\n"+getClass().getSimpleName()+" does not support stdin;"
//							+ " please specify all input files on the command line "
//							+ " (wildcard is allowed, e.g. 'pileup.sh *.sam'),"
//							+ " or use pileup_old.sh for streamed input.\n");
		}
...
//					else{outstream.println(scaf.name+", "+scaf.length+", "+len);}
				...
				scaf=table.get(name);
				if(ADD_FROM_REF && scaf==null){//TODO: This needs to calculate the scaffold length...
					scaf=new Scaffold(name, 0);
					assert(false) : "SAM header did not include "+name;
//					if(prealloc && USE_COVERAGE_ARRAYS) {
//						scaf.obj1=makeCA(scaf.length);
//						if(STRANDED){scaf.obj2=makeCA(scaf.length);}
//					}
					if(!warned){
...
//				tsw.print(Tools.format("%s\t%d\t%d\t%.4f\t%d\t%.4f\t%d\t%.4f\n",s,len,bases,bases*invlen,reads,reads*readMult2,frags,frags*fragMult2));
	...
	class LoadThread extends Thread {
		...
		private void processViaStreamer(String fname){
			final boolean processHeader=(stdin && true);
