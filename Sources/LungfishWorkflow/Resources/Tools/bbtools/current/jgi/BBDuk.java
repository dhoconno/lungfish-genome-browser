...
//			outstream.println("NOTE: No reference files specified, no trimming mode, no min avg quality, no histograms - read sequences will not be changed.");
		...
		align=(align || alignOut!=null);
		if(align) {
			sidechannel=new SideChannel3(alignRef, alignOut, alignK1, alignK2, 
	...
	public static String modifyRefPath(String s){
		if(s==null || Tools.isReadableFile(s)){
			...
			if("phix".equalsIgnoreCase(s)){
				...
				s=Data.findPath("?phix_adapters.fa.gz");
	...
	private void spawnProcessThreads(Timer t){
		...
		if(skipreads>0){
			...
			if(reads==null || reads.isEmpty()){
				ReadWrite.closeStreams(cris, ros, rosb, ross, sidechannel==null ? null : sidechannel.cros);
		...
		errorState|=ReadWrite.closeOutputStreams(ros, rosb, ross, sidechannel==null ? null : sidechannel.cros);
...
//								assert(false) : trimClip+", "+x;
				...
				if(sidechannel!=null) {
					sidechannel.write(sideList, ln.id);
					sideList.clear();
				}
